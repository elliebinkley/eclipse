...
template<class T>
int SimpleList<T>::add( const T& data )
{
    ...
    if( m_head )
    {
        while( elementPtr )
        {
            ...
            if( nodeData == data ) return 1; // duplicate
            if( ! elementPtr->getNext() ) break;
...
template<class T>
int SimpleList<T>::remove( const T& data )
{
    int retVal = 1;
    Element* elementPtr = m_head;
    while( elementPtr )
...
template<class T>
T SimpleList<T>::find( const T& data ) const
{
    Element* elementPtr = m_head;
    while( elementPtr )
    {
