...
Person::Person(const std::string& firstName, const std::string& lastName,
		...
		m_firstName(firstName), m_lastName(lastName), m_address(address), m_ssn(
				ssn) {
}

Person::Person(const char* firstName, const char* lastName, const char* address,
		const SSN* ssn) :
		m_firstName(firstName), m_lastName(lastName), m_address(address), m_ssn(
				*ssn) {
}
...
void Person::print() {
	cout << "name:" << m_firstName << " " << m_lastName << " ssn:"
			<< m_ssn.getSSN() << " address:" << m_address << endl;
}
...
bool operator==(const SSN& ssn1, const SSN& ssn2) {
	return (ssn1.m_firstPart == ssn2.m_firstPart
			&& ssn1.m_secondPart == ssn2.m_secondPart
