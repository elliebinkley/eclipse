...
// run all tests in this suite
  void
  UniformInitializationSyntax::runTests()
  {
    arrayInit ();
    memberArrayInit_0 ();
    memberArrayInit_1 ();
...
// see comments above for arrayInit()
  void
  UniformInitializationSyntax::arrayInit()
  {
    T_START;
    ...
    {
    public:
      A (const int value, const std::string& description) :
	     m_a (value), m_desc (description)
         {};
    ...
    // C++11 only.
    A* a = new A[3]   { A (3, std::string ("A0")), A (4, std::string ("A1")),
    	                A ( 5, std::string ("A2")) };
    for (int i = 0; i < 3; i++)
    {
  ...
  void
  UniformInitializationSyntax::memberArrayInit_0()
  {
    T_START;
...
// Same as memberArrayInit_0() but use std::array as a member.
  void
  UniformInitializationSyntax::memberArrayInit_1 ()
  {
    T_START;
...
// in C++11 containers can be initialized with brackets.
  void
  UniformInitializationSyntax::containerIntialization ()
  {
    T_START;
    class A
    {
...
// in C++11, members can be initialized in the class declaration
  void
  UniformInitializationSyntax::inClassInitialization ()
  {
    T_START;
    class A
    {
