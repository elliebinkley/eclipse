...
int main()
{
    ...
    // create a key for the main thread; the key can be used by each thread. Put this key in a vector of keys; note the vector will
    ...
    std::stringstream s;
    s << "END:  Function=" << __FUNCTION__ << " File=" << __FILE__ << " Line=" << __LINE__;
    std::cout << s.str() << endl;
...
doSomething( void* data )
{
     ...
     */
    std::stringstream s4;
    s4 << "END:  Function=" << __FUNCTION__ << " File=" << __FILE__ << " Line=" << __LINE__
            << " threadID=" << id;
