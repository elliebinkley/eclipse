...
public:
  ...
  inline friend bool
  operator== (const A1& lhs, const A1& rhs);
  inline virtual std::string
  toString () const
  {
    return std::string (m_a1);
  }
  ;
protected:
  char* m_a1;
...
public:
  inline
  A (const std::string& s, const B& b, const std::string& a1);
  inline
  A (const A& a);
  inline
  ~A ();
...
A::A (const std::string& s, const B& b, const std::string& a1) :
    A1 (a1), m_a (s)
{
  T_START
  ;
...
{
public:
  Carnivore (Gender gender, std::string prey) :
      Mammal (gender), m_prey (prey)
  {
    T_START
    ;
    cout << "Carnivore(" << m_genderMap.at (gender) << "prey=" << m_prey << ")"
	<< endl;
    T_END
  }
  ;
...
{
public:
  Herbivore (Gender gender, std::string vegetable) :
      Mammal (gender), m_vegetable (vegetable)
  {
    T_START
    ;
    cout << "Herbivore(" << m_genderMap.at (gender) << ", vegetable="
	<< vegetable << ")" << endl;
    T_END
  }
  ;
...
public:
      ...
      Mammal (gender)
  {
    T_START
    ;
    cout << "Bear(" << m_genderMap.at (sex ()) << " prey=" << prey
	<< " vegetable=" << vegetable << ")" << endl;
    T_END
  }
  ;
  std::string
